...
public class ForeServlet extends  BaseForeServlet{
    ...
    //商家注册
    public String merregister(HttpServletRequest request, HttpServletResponse response, Page page){
        try{
            String name = new String(request.getParameter("name").getBytes("ISO-8859-1"),"UTF-8");
            String password = request.getParameter("password");
            String email = request.getParameter("email");
            String placename = new String(request.getParameter("placename").getBytes("ISO-8859-1"),"UTF-8");
            name = HtmlUtils.htmlEscape(name);
            boolean exist = merDAO.isExist(name);
            if (exist) {
                request.setAttribute("msg", "*用户名已经被使用！");
                return "register.jsp";
            }
    ...
    //付款及修改状态
    public String userReserve(HttpServletRequest request, HttpServletResponse response, Page page) {
        int rid = Integer.parseInt(request.getParameter("rid"));
        Reserve reserve = new Reserve();
        reserve.setId(rid);
        //订单状态修改为已支付
        reserveDAO.edit(rid, "已退款");
        //场地安排表项状态修改为"审核中"
        reserveItemDAO.fill(reserve);
        for (int i = 0; i < reserve.getReserveItemList().size(); i++) {
            stadiumPlanItemDAO.edit(reserve.getReserveItemList().get(i).getId(), "已退款");
        }
        return "@foreuser";
    }
